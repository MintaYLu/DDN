#' ddn
#'
#' ddn: Differential Dependency Networks to detect significant rewiring of regulatory correlation structures under different biological conditions
#'
#' @param data1 gene expression data under condition 1
#' @param data2 gene expression data under condition 2
...
#'
#' @return beta.matrix (the network coefficient parameter matrix)
#'
#'
#' @export
ddn <- function(data1, data2, lambda1, lambda2) {
  ...
  if(any(is.na(data1))) stop("data1 cannot have missing values!")
  if(any(is.na(data2))) stop("data2 cannot have missing values!")
  if(ncol(data1) != ncol(data2)) stop("data1 and data2 should have the same feature number")
